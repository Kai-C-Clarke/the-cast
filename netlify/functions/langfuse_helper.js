...
function sendLangfuseTrace({ name, input, output, model, usage, metadata }) {
  ...
  if (!secret || !pub) return Promise.resolve();
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode !== 207 && res.statusCode !== 200) {
          console.log(`[langfuse] status ${res.statusCode}: ${data.slice(0, 300)}`);
        } else {
          console.log(`[langfuse] trace sent: ${name} (${res.statusCode})`);
        }
        resolve();
      });
    });
    req.on('error', (e) => {
      console.log(`[langfuse] error: ${e.message}`);
      resolve();
    });
    req.write(body);
    req.end();
